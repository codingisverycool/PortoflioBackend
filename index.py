...
app = Flask(__name__)
CORS(app, supports_credentials=True, origins=["turiyaportfolioplatform-9v5cm5rvi-aaryans-projects-56d3a379.vercel.app"])
...
# Generate horizontal bar chart and return base64 PNG
def generate_bar_chart(df, value_col, label_col, title):
    plt.figure(figsize=(8, 5))
    df = df.sort_values(value_col)
    colors = ['green' if x >= 0 else 'red' for x in df[value_col]]
    plt.barh(df[label_col], df[value_col], color=colors)
...
# Generate treemap and return base64 PNG
def generate_treemap(df, size_col, label_col, color_col, title):
    plt.figure(figsize=(12, 8))
    max_gain = max(df[color_col].max(), 0)
    max_loss = min(df[color_col].min(), 0)
    colors = []
    for _, row in df.iterrows():
        if row[color_col] > 0:
            intensity = 1.0 - 0.7 * (row[color_col] / max_gain if max_gain > 0 else 0)
            colors.append((0, 0.3 + intensity * 0.7, 0, 0.7))
        else:
            intensity = 0.3 + 0.7 * (abs(row[color_col]) / abs(max_loss) if max_loss < 0 else 0)
            colors.append((intensity, 0, 0, 0.7))
    sq.plot(
        sizes=df[size_col],
        label=[f"{row[label_col]}\n${row[size_col]/1000:.1f}k" for _, row in df.iterrows()],
        color=colors,
        alpha=0.7,
...
def portfolio_tracker_api():
    ...
    try:
        ...
        plt.pie(df['AllocationPercent'], labels=df['Ticker'], autopct='%1.1f%%')
        plt.title('Stock Allocation')
        buf = io.BytesIO()
        plt.savefig(buf, format='png')
        buf.seek(0)
        imgData['stockPie'] = base64.b64encode(buf.getvalue()).decode('utf-8')
        plt.close()

        # Pie Chart: Sector
        plt.figure(figsize=(6, 6))
        plt.pie(sector_values.values(), labels=sector_values.keys(), autopct='%1.1f%%')
        plt.title('Sector Allocation')
        buf = io.BytesIO()
        plt.savefig(buf, format='png')
        buf.seek(0)
        imgData['sectorPie'] = base64.b64encode(buf.getvalue()).decode('utf-8')
        plt.close()

        # Gain Bar
        plt.figure(figsize=(8, 4))
        df.sort_values('Gain/Loss', inplace=True)  # FIXED: use correct column name
        colors = ['green' if g >= 0 else 'red' for g in df['Gain/Loss']]
        plt.barh(df['Ticker'], df['Gain/Loss'], color=colors)
        plt.title('Gain/Loss')
        buf = io.BytesIO()
        plt.savefig(buf, format='png')
        buf.seek(0)
        imgData['gainBar'] = base64.b64encode(buf.getvalue()).decode('utf-8')
        plt.close()
        ...
        squarify.plot(
            sizes=df['Current Value'],
            label=[f"{row['Ticker']} (${row['Current Value']/1000:.1f}k)" for _, row in df.iterrows()],
            color=colors,
            alpha=0.7
        )
        plt.title("Treemap")
        ...
        plt.savefig(buf, format='png')
        buf.seek(0)
        imgData['treemap'] = base64.b64encode(buf.getvalue()).decode('utf-8')
        plt.close()
